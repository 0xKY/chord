...
public class CBoatItem extends Item {
    private static final Predicate<Entity> RIDERS = EntityPredicates.EXCEPT_SPECTATOR.and(Entity::isCollidable);
    ...
    private CBoatEntity createBoat(World world, double x, double y, double z) {
        CBoatEntity entity = boatSupplier.get().create(world);
        if (entity != null) {
            entity.setPos(x, y, z);
